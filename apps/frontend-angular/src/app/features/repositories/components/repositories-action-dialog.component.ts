...
import {
  exportRepoReportText,
  type RepoActionReport,
  type RepoOpSection,
  type RepoOpStatus,
} from '../utils/repositories-action-reports.util'

const statusLabel: Record<RepoOpStatus, string> = { ok: 'Completado', warn: 'Atención', fail: 'Fallo' }
const statusIcon: Record<RepoOpStatus, string> = { ok: 'check_circle', warn: 'warning', fail: 'error' }

const providerLabel: Record<string, string> = {
  github: 'GitHub',
  gitlab: 'GitLab',
  multi: 'Multi-proveedor',
}

@Component({
  selector: 'app-repositories-action-dialog',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [MatDialogModule, MatButtonModule, MatIconModule],
  template: `
    <div class="repo-op-dialog">
      <header class="repo-op-dialog__hero">
        <div class="repo-op-dialog__icon" [attr.data-status]="data.status">
          <mat-icon>{{ data.actionId === 'payload' ? 'data_object' : 'folder_special' }}</mat-icon>
        </div>
        <div>
          <div class="repo-op-dialog__chips">
            @if (data.provider) {
              <span class="chip">{{ providerLabel[data.provider] ?? data.provider }}</span>
            }
            <span class="chip chip--muted">{{ data.sectionId }}</span>
          </div>
  ...
  `,
  styles: `
    .repo-op-dialog { --accent: #6366f1; --accent-deep: #4338ca; }
    .repo-op-dialog__hero {
    ...
    .repo-op-dialog__icon {
      width: 42px; height: 42px; border-radius: 10px; display: grid; place-items: center;
      background: color-mix(in srgb, var(--accent) 14%, transparent);
      border: 1px solid color-mix(in srgb, var(--accent) 28%, transparent);
    }
    .repo-op-dialog__icon mat-icon { color: var(--accent-deep); }
    .repo-op-dialog__chips { display: flex; gap: 0.3rem; margin-bottom: 0.2rem; }
...
export class RepositoriesActionDialogComponent {
  readonly data = inject<RepoActionReport>(MAT_DIALOG_DATA)
  private readonly toast = inject(ToastService)
  readonly statusIcon = statusIcon
  readonly statusLabel = statusLabel
  readonly providerLabel = providerLabel

  isCodeBlock = (s: RepoOpSection): boolean => /payload|log|json|url|terminal|commit/i.test(s.title)

  formatMs = (ms: number): string => (ms >= 1000 ? `${(ms / 1000).toFixed(1)} s` : `${ms} ms`)

  handleExport = (): void => {
    const text = exportRepoReportText(this.data)
