...
export const SETTINGS_SYNC_HISTORY = [12, 18, 14, 22, 16, 19, 15, 21, 17, 20, 14, 18]

export const SETTINGS_INTEGRATIONS: SettingsIntegration[] = [
  {
    id: 'slack',
    label: 'Slack',
    desc: 'Alertas y notificaciones a canales #cloudops-alerts',
  ...
  },
  {
    id: 'pagerduty',
    label: 'PagerDuty',
    desc: 'Escalado automático de incidentes críticos P1/P2',
  ...
  },
  {
    id: 'jira',
    label: 'Jira',
    desc: 'Sincronización bidireccional de tickets de cambio',
    category: 'ITSM',
    icon: 'confirmation_number',
    enabled: false,
    status: 'disconnected',
    lastSync: ts(86400 * 14),
    events24h: 0,
    accent: '#0052CC',
  },
  {
    id: 'servicenow',
    label: 'ServiceNow',
    desc: 'CMDB, change management y aprobaciones',
    category: 'ITSM',
    icon: 'business',
    enabled: false,
    status: 'pending',
  ...
  },
  {
    ...
    desc: 'Notificaciones a canal de operaciones',
    category: 'Comunicación',
    icon: 'groups',
    enabled: true,
    status: 'connected',
  ...
  },
  {
    id: 'github',
    label: 'GitHub',
    desc: 'Webhooks de despliegue y estado de pipelines',
    category: 'DevOps',
    icon: 'code',
    enabled: true,
    status: 'connected',
...
export const SETTINGS_PLATFORM_SOURCES = [
  { id: 'alerts', label: 'Alertas', module: 'AlertsModule', events: ['alert.critical', 'alert.warning'] },
  { id: 'jenkins', label: 'Jenkins CI/CD', module: 'JenkinsModule', events: ['deploy.success', 'deploy.failed', 'workflow.run'] },
  { id: 'terraform', label: 'Terraform', module: 'TerraformModule', events: ['deploy.success', 'deploy.failed', 'change.create'] },
  { id: 'github', label: 'GitHub Deployments', module: 'GithubModule', events: ['deploy.success', 'deploy.failed', 'workflow.run'] },
  { id: 'vps', label: 'VPS / Backups', module: 'VpsModule', events: ['backup.completed', 'ops.completed'] },
  { id: 'command-center', label: 'Command Center', module: 'CommandCenterModule', events: ['ops.completed', 'change.create'] },
  { id: 'settings', label: 'Integraciones', module: 'IntegrationsModule', events: ['integration.enabled', 'integration.test'] },
] as const
...
export const SETTINGS_NOTIF_HISTORY = [8, 14, 11, 22, 18, 25, 16, 20, 12, 28, 19, 24, 15, 21]

export const SETTINGS_SECURITY_POLICIES: SettingsSecurityPolicy[] = [
  {
    id: 'mfa',
    label: 'MFA obligatorio para admins',
    desc: 'Requiere TOTP o WebAuthn en cada inicio de sesión administrativo',
    icon: 'verified_user',
    enabled: true,
    severity: 'critical',
    impact: '12 admins · 100% cumplimiento',
  },
  {
    id: 'session',
    label: 'Timeout de sesión (8 h)',
    desc: 'Cierra sesiones inactivas automáticamente tras 8 horas',
    icon: 'schedule',
    enabled: true,
    severity: 'high',
    impact: '47 sesiones activas',
  },
  {
    id: 'ip',
    label: 'Lista blanca de IP',
    desc: 'Restringe acceso a rangos corporativos 10.0.0.0/8 y VPN',
    icon: 'vpn_lock',
    enabled: false,
    severity: 'medium',
    impact: '0 rangos configurados',
  },
  {
    id: 'password',
    label: 'Política de contraseñas',
    desc: 'Mínimo 12 caracteres, rotación cada 90 días, sin reutilización',
    icon: 'password',
    enabled: true,
    severity: 'high',
    impact: 'NIST SP 800-63B',
  },
  {
    id: 'audit',
    label: 'Auditoría extendida',
    desc: 'Registra lecturas sensibles y exportaciones de datos',
    icon: 'receipt_long',
    enabled: true,
    severity: 'medium',
    impact: 'Retención 365 días',
  },
]
