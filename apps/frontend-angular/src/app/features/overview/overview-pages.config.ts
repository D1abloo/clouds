...
export const COMMAND_CENTER_QUICK_ACTIONS: CommandCenterQuickAction[] = [
  {
    ...
    provider: 'AWS',
    logo: 'aws',
    resource: 'aws-prod-app-1',
    region: 'us-east-1',
    detail: 'EC2 t3.large · reinicio en caliente',
    eta: '3 min',
    effect: 'Reinicia la instancia EC2 aws-prod-app-1 sin reprovisionar: detiene servicios, reinicia el SO y valida health checks antes de devolver tráfico.',
    steps: [
      'Drena conexiones del balanceador (30 s)',
      'Reinicio del SO vía API EC2',
    ...
    destinationHint: 'Verás el estado «running», uptime reiniciado y logs del reinicio en el detalle de aws-prod-app-1.',
  ...
  },
  {
    ...
    region: 'prod-cluster',
    detail: 'Deployment checkout-api · réplicas 3 → 5',
    eta: '2 min',
    effect: 'Escala horizontalmente el deployment checkout-api en prod-cluster de 3 a 5 réplicas y espera readiness de los nuevos pods.',
    steps: [
      'Patch del deployment (replicas: 5)',
      'Programación de 2 pods nuevos',
      'Validación readiness + HPA estable',
    ],
    destinationRoute: '/kubernetes/pods',
    destinationLabel: 'Kubernetes → Pods',
    destinationHint: 'Filtra por namespace checkout: verás 5 pods activos del deployment checkout-api con estado Running.',
  ...
  },
  {
    ...
    region: 'global',
    detail: 'Workspace aws-production · dry-run',
    eta: '5 min',
    effect: 'Ejecuta terraform plan en el workspace aws-production: compara estado remoto vs código y genera un informe de cambios sin aplicar nada.',
    steps: [
      'Init remoto + refresh del state',
      'Plan contra 12 recursos gestionados',
      'Genera diff (+2 crear, ~1 modificar) en el run',
    ],
    destinationRoute: '/terraform/workspaces',
    destinationLabel: 'Terraform → Workspaces',
    destinationHint: 'Abrirás el workspace aws-production con el último run de plan, diff de recursos y botón para apply.',
  ...
  },
  {
    ...
    region: 'ci-01',
    detail: 'Job deploy-staging · build + deploy',
    eta: '8 min',
    effect: 'Dispara el pipeline deploy-staging en Jenkins ci-01: build, tests, empaquetado y despliegue al entorno staging.',
    steps: [
      'Checkout + build #843',
      'Tests unitarios e integración',
      'Deploy a staging vía artefacto',
    ],
    destinationRoute: '/jenkins/jobs',
    destinationLabel: 'Jenkins → Jobs',
    destinationHint: 'Verás el job deploy-staging en ejecución con stages, logs en vivo y enlace al entorno desplegado.',
  ...
  },
  {
    ...
    resource: 'vps-fleet',
    region: 'fra1',
    detail: 'Snapshot completo · 6 hosts en fra1',
    eta: '12 min',
    effect: 'Crea snapshots consistentes de los 6 VPS del pool vps-fleet en fra1 (SO, volúmenes y metadatos de configuración).',
    steps: [
      'Quiesce de servicios stateful',
      'Snapshot por host (paralelo x3)',
      'Verificación de integridad + registro en catálogo',
    ],
    destinationRoute: '/vps/overview',
    destinationLabel: 'VPS → Overview',
    destinationHint: 'En la flota VPS verás el timestamp del último backup, tamaño del snapshot y estado por host.',
  ...
  },
  {
    ...
    region: 'us-central1',
    detail: 'Proyecto gcp-analytics · 2 proyectos, 6 VMs',
    eta: '4 min',
    effect: 'Sincroniza el inventario cloud del proyecto gcp-analytics: VMs, discos, redes y metadatos de coste con la API de GCP.',
    steps: [
      'Listado de Compute Engine por proyecto',
      'Mapeo de etiquetas y costes estimados',
      'Actualización del índice del explorador de recursos',
    ],
    destinationRoute: '/cloud/gcp/instances',
    destinationLabel: 'Cloud GCP → Instancias',
    destinationHint: 'Llegarás al listado GCP actualizado con las 6 instancias, regiones y estado de sincronización reciente.',
    actionType: 'sync-inventory',
  },
]

export const COMMAND_CENTER_ACTION_PRESETS = [
...
export const HEALTH_TIMELINE = [
  { time: ts(3), event: 'Pod worker-crash-loop entra en CrashLoopBackOff', severity: 'critical' as const },
  { time: ts(12), event: 'Jenkins integration-tests #841 falla en stage E2E', severity: 'critical' as const },
  { time: ts(18), event: 'Disco azure-db-5 supera umbral 85%', severity: 'warning' as const },
  { time: ts(35), event: 'Backup snap-staging falla en host fra1-vps-03', severity: 'critical' as const },
  { time: ts(55), event: 'CPU aws-prod-app-3 > 90% durante 5 min', severity: 'warning' as const },
]
...
export const HEALTH_CATEGORIES = [
  { category: 'Instancias cloud', healthy: 88, warning: 4, critical: 0, icon: 'dns' },
  { category: 'Contenedores / K8s', healthy: 36, warning: 3, critical: 1, icon: 'hub' },
  { category: 'CI/CD', healthy: 12, warning: 0, critical: 1, icon: 'build' },
  { category: 'Backups', healthy: 11, warning: 1, critical: 1, icon: 'backup' },
  { category: 'Red & balanceo', healthy: 24, warning: 0, critical: 0, icon: 'device_hub' },
]
...
export const EXPLORER_TYPE_FILTERS = [
  { key: '', label: 'Todos', count: EXPLORER_RESOURCES_RICH.length },
  { key: 'instance', label: 'Instancias', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'instance').length },
  { key: 'kubernetes', label: 'Kubernetes', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'kubernetes').length },
  { key: 'docker', label: 'Docker', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'docker').length },
  { key: 'jenkins', label: 'Jenkins', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'jenkins').length },
  { key: 'terraform', label: 'Terraform', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'terraform').length },
  { key: 'account', label: 'Cuentas', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'account').length },
  { key: 'network', label: 'Red', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'network').length },
  { key: 'alert', label: 'Alertas', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'alert').length },
  { key: 'vps', label: 'VPS', count: EXPLORER_RESOURCES_RICH.filter((r) => r.typeKey === 'vps').length },
]
